...
main = do
  ...
  case winMaybe of
    ...
    Just window -> do
      ...
      uLoc_A_lightSpaceMatrix    <- get $ GL.uniformLocation lightingShader "lightSpaceMatrix"
      uLoc_A_shadowMap           <- get $ GL.uniformLocation lightingShader "shadowMap"
      ...
      GL.uniform uLoc_A_shadowMap         $= (2::GLint)
        ...
        if b then
          ...
          case t_maybe of
            Just t -> do
              ...
              GL.uniform uLoc_A_lightSpaceMatrix   $= lightSpaceMatrix

              GL.activeTexture $= GL.TextureUnit 2
              GL.textureBinding GL.Texture2D $= Just depthMap

              renderScene modelData (uLoc_A_model, Nothing, Nothing, Just uLoc_A_material_shininess) 
