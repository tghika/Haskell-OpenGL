...
main = do
  ...
  case winMaybe of
    ...
    Just window -> do
      ...
      lightCubeShader <- buildShaderProgram window ["./light_cube.vs", "./light_cube.fs"]
      ...
      -- uniformLocations
      uLoc_A_objectColor <- get $ GL.uniformLocation lightingShader "objectColor"
      uLoc_A_lightColor  <- get $ GL.uniformLocation lightingShader "lightColor"
      uLoc_A_lightPos    <- get $ GL.uniformLocation lightingShader "lightPos"
      ...
      GL.currentProgram $= Just lightCubeShader
      -- AttribLocation
      aLoc_B_aPos        <- get $ GL.attribLocation  lightCubeShader "aPos"
      -- uniformLocations
      uLoc_B_model       <- get $ GL.uniformLocation lightCubeShader "model"
      uLoc_B_view        <- get $ GL.uniformLocation lightCubeShader "view"
      uLoc_B_projection  <- get $ GL.uniformLocation lightCubeShader "projection"
      ...
      GL.vertexAttribPointer aLoc_A_aPos
        $= (GL.ToFloat, GL.VertexArrayDescriptor 3 GL.Float (v_layout!!2) (plusPtr nullPtr . fromIntegral $ v_layout!!0))
      GL.vertexAttribArray aLoc_A_aPos $= GL.Enabled
      GL.vertexAttribPointer aLoc_A_aNormal
        $= (GL.ToFloat, GL.VertexArrayDescriptor 3 GL.Float (v_layout!!2) (plusPtr nullPtr . fromIntegral $ v_layout!!1))
      GL.vertexAttribArray aLoc_A_aNormal $= GL.Enabled

      lightCubeVAO <- genObjectName
      GL.bindVertexArrayObject $= Just lightCubeVAO
      GL.vertexAttribPointer aLoc_B_aPos
        $= (GL.ToFloat, GL.VertexArrayDescriptor 3 GL.Float (v_layout!!2) (plusPtr nullPtr . fromIntegral $ v_layout!!0))
      GL.vertexAttribArray aLoc_B_aPos $= GL.Enabled
          ...
          case t_maybe of
            Just t -> do
              ...
              model2 <- mat4ToGLmatrix $
                 mat4_translate currentLightPos !*! mat4_rotate (V3 0.3 0.2 0.1) (realToFrac t * radians (-55)) !*! mat4_scale 0.2 0.2 0.2

              GL.currentProgram $= Just lightingShader

              GL.uniform uLoc_A_objectColor $= (Vector3 1 0.5 0.31 :: Vector3 GLfloat)
              GL.uniform uLoc_A_lightColor  $= (Vector3 1 1 1 :: Vector3 GLfloat)
              GL.uniform uLoc_A_lightPos    $= v3ToVector3 (currentLightPos)
      ...
      deleteObjectName lightCubeVAO
      deleteObjectName vbo
      deleteObjectName lightingShader
      deleteObjectName lightCubeShader
